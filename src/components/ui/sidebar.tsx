...
>(
  (
    ...
    const getInitialWidth = () => {
      if (typeof document !== "undefined") {
        ...
        const widthCookie = cookies.find(cookie => 
          cookie.trim().startsWith(`${SIDEBAR_WIDTH_COOKIE_NAME}=`)
        )
        if (widthCookie) {
    ...
    const [width, setWidthState] = React.useState(getInitialWidth)
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "fixed top-16 bottom-0 z-10 hidden h-[calc(100vh-4rem)] w-[--sidebar-width] md:flex",
...
>(({ side = "left", className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "absolute top-0 bottom-0 w-1 cursor-col-resize bg-transparent hover:bg-border transition-colors group",
    ...
    >
      {/* Visual indicator */}
      <div className="absolute inset-y-0 w-0.5 bg-border opacity-0 group-hover:opacity-100 transition-opacity" 
