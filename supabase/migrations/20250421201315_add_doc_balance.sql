/*
  # Add Document Currency Balance Function

  1. Changes
    - Add get_cash_book_doc_balance function to calculate balance in document currency
    - Ensure it returns the most up-to-date balance
    - Handle currency code type casting
*/

-- Drop existing function if exists
DROP FUNCTION IF EXISTS get_cash_book_doc_balance(uuid);

-- Create document currency balance function
CREATE OR REPLACE FUNCTION get_cash_book_doc_balance(p_account_id uuid)
RETURNS TABLE (
  balance numeric,
  currency_id uuid,
  currency_code text
